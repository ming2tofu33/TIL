...
SELECT 
    ...
    DATE_FORMAT(birth, '%w') AS 요일_번호,
    DATE_FORMAT(birth, '%W') AS 요일_이름,
    DATE_FORMAT(birth, '%Y,%y,%M,%m,%D,%d')
FROM dt_demo;
...
SELECT
	...
	DATE_SUB(CURRENT_DATE, INTERVAL 1 MONTH) AS 한달전
FROM dt_demo;
