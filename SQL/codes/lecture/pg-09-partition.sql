...
WITH cp_sales AS (
	SELECT
		...
		p.price AS 상품가격, 
		COUNT(o.product_id) AS 해당상품의주문건수, 
		SUM(o.quantity) AS 해당상품판매개수, 
		SUM(o.amount) AS 해당상품총매출
	FROM products p
...
cp_rank AS (
	SELECT
		*,
		RANK() OVER (PARTITION BY 카테고리 ORDER BY 해당상품총매출 DESC) AS 매출순위,
		DENSE_RANK() OVER (PARTITION BY 카테고리 ORDER BY 해당상품판매개수 DESC) AS 판매량순위
	FROM cp_sales
)
SELECT
	카테고리, 상품id, 상품이름, 상품가격, 해당상품의주문건수, 해당상품판매개수, 해당상품총매출, 매출순위, 판매량순위
FROM cp_rank
WHERE 매출순위 <= 5
ORDER BY 카테고리, 매출순위;
